import os
import sys
fpath = os.path.join(os.path.dirname(__file__), '../fdint/_gfd.pyx')
...
cdef inline double gfd{k2}h(double phi, double beta):'''.format(k2=k2))
        for phi_min, phi_max, ext in [(-INF, -2e0,    '_lt_m2'),
            ...
            if phi_max != INF:
                ...
                f.write('''
    if phi < {phi_max:.1f}:
        return gfd{k2}h{ext}(phi, beta)'''.format(k2=k2,ext=ext,phi_max=phi_max))
            else:
                f.write('''
    return gfd{k2}h{ext}(phi, beta)
'''.format(k2=k2,ext=ext,phi_max=phi_max))
    
    
    # Generate `gfd1h_lt_m2`, etc.
    for i in xrange(-1,6,2):
        k2 = str(i).replace('-','m')
        for phi_min, phi_max, ext in [(-INF, -2e0,    '_lt_m2'),
                                      (-2e0,  0e0,  '_m2_to_0'),
            ...
            f.write('''
@cython.cdivision(True)
cdef inline double gfd{k2}h{ext}(double phi, double beta):'''
    ...
    if(beta <= BS{k2}h{ext}__{m}):
        return gfd{k2}h{ext}__{m}(phi, beta)'''.format(k2=k2, ext=ext, m=m))
                if m % 2 == 1:
                    last_odd_m = m
            if phi_max != INF:
                f.write('''
    warnings.warn('gfd{k2}h: less than 24 bits of accuracy',
                  RuntimeWarning)
    # odd-order approximations (overestimation) result in better stability
    # for solid-state physics simulations
    return gfd{k2}h{ext}__{m}(phi, beta)
'''.format(k2=k2, ext=ext, m=last_odd_m))
            else:
                f.write('''
    warnings.warn('gfd{k2}h: 24 bits of accuracy not guaranteed',
                  RuntimeWarning)
    # odd-order approximations (overestimation) result in better stability
    # for solid-state physics simulations
    return gfd{k2}h{ext}__{m}(phi, beta)
    ...
    for i in xrange(-1,6,2):
        ...
        for phi_min, phi_max, ext in [(-INF, -2e0,    '_lt_m2'),
            ...
            for m, _ in enumerate(xrange(i, 22, 2)):
                ...
                f.write('''
@cython.cdivision(True)
cdef inline double gfd{k2}h{ext}__{m}(double phi, double beta):'''
                ...
                for n, _k2 in enumerate(xrange(i, 22, 2)):
                    ...
                    if n == 0:
                        f.write('''
        return (       G0 *fd{k2}h{ext}(phi)'''.format(k2=_k2, ext=ext))
                    else:
                        mstr = str(m).replace('10','A')
                        nstr = str(n).replace('10','A')
                        f.write('''
                +beta*(G{m}{n}*fd{k2}h{ext}(phi)'''.format(k2=_k2, ext=ext,
                                                           m=mstr,
                                                           n=nstr,
                                                           ))
                f.write('\n                )'+')'*m+'\n')